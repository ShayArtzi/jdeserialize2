...
public class ExceptionState extends ContentBase {
    ...
    public ExceptionState(Content exobj, byte[] data) {
        super(ContentTypeEnum.EXCEPTIONSTATE);
    ...
    public String toString() {
        ...
        if(streamdata.length > 0) {
            for(int i = 0; i < streamdata.length; i++) {
                if((i % 16) == 0) {
                    sb.append(FormatUtil.getLineSeperator()).append(String.format("%7x: ", Integer.valueOf(i)));
                }
                sb.append(" ").append(FormatUtil.hexnoprefix(streamdata[i]));
            }
